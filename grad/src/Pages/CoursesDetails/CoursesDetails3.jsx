...
const CoursesDetails3 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (fetchedData.level3 && fetchedData.level3.term1 && fetchedData.level3.term1.cs) {
                    setCs(fetchedData.level3.term1.cs);
                }
                if (fetchedData.level3 && fetchedData.level3.term1 && fetchedData.level3.term1.is) {
                    setIs(fetchedData.level3.term1.is);
                }
                if (fetchedData.level3 && fetchedData.level3.term1 && fetchedData.level3.term1.it) {
                    setIt(fetchedData.level3.term1.it);
                }
                if (fetchedData.level3 && fetchedData.level3.term1 && fetchedData.level3.term1.mm) {
                    setMm(fetchedData.level3.term1.mm);
                }
                if (fetchedData.level3 && fetchedData.level3.term2 && fetchedData.level3.term2.cs) {
                    setCs2(fetchedData.level3.term2.cs);
                }
                if (fetchedData.level3 && fetchedData.level3.term2 && fetchedData.level3.term2.is) {
                    setIs2(fetchedData.level3.term2.is);
                }
                if (fetchedData.level3 && fetchedData.level3.term2 && fetchedData.level3.term2.it) {
                    setIt2(fetchedData.level3.term2.it);
                }
                if (fetchedData.level3 && fetchedData.level3.term2 && fetchedData.level3.term2.mm) {
                    setMm2(fetchedData.level3.term2.mm);
                }
            } catch (error) {
                console.log(error);
            } finally {
    ...
    return (
        <div className="bg-[#b8d0f2] py-6 mt-16 text-right">
            <ScrollToTop />
            <Header title={'مواد الفرقة الدراسية الثالثة'} />
            ...
            <div className="text-right bg-white border rounded-2xl pt-4 text-[#0060E4] font-medium mx-60 pb-8">
                ...
                {cs && cs.map((course, index) => (
                    <div key={index}>
                        ...
                        <p className='text-2xl mr-5'>{course.prerequesetcourse.join(" + ")}{course.prerequesetcourse.length === 0 ? "لا تعتمد على شئ" : " تعتمد على" }</p>
                    </div>
                ))}
