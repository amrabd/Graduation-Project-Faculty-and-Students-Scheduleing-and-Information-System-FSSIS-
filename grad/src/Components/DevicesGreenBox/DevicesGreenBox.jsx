...
const DevicesGreenBox = ({ img, name, no , id}) => {
    ...
    return (
        <div className='bg-[#00A300] h-96 w-80  rounded-xl flex justify-center items-center flex-col pt-2'>
            <ToastContainer
            ...
            <form onSubmit={handleSubmit} className='ml-[10px]'>
                <select className='py-2 bg-[#00A300] border-solid border-white border-2 focus:outline-none rounded-lg px-1 outline-2 text-white font-bold' id="optionGroup" name="optionGroup" value={selectedOption} onChange={handleChange}>
                    <optgroup>
                ...
                <button type="submit" className={`inline mx-auto ${selectedOption === '' ? " cursor-not-allowed" :  "hover:bg-white hover:text-black"} bg-[#046404] mt-4 ml-5`}>Submit</button>
            </form>
            </div>
        </div>
    )
